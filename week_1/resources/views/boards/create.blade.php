...
@section('content')
    <div class="w-full max-w-screen-lg shadow rounded p-8">
        <p class="text-3xl font-bold leading-7 text-center">글 작성</p>
        <form action="{{ route('boards.write')  }}" method="post">
            @csrf
            ...
            @enderror
            <div>
                <div class="w-full flex flex-col mt-8 @error('title') has-danger @enderror">
                    <label class="font-semibold leading-none">내용</label>
                    <textarea type="text"
                              name="body"
                              class="h-40 text-base leading-none text-gray-900 p-3 focus:oultine-none focus:border-blue-700 mt-4 bg-gray-100 border rounded border-gray-200">{{ old('body') }}</textarea>
                </div>
            </div>
            @error('body')
            <div class="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800"
